/* Files tab — real volume operations incl. zip/unzip and drag-and-drop. Scoped
   to the service from the path. */
(function () {
  window.ServiceTabs = window.ServiceTabs || {};

  window.ServiceTabs.files = async function ({ content, server }) {
    ...
    function create(t) {
      const label = t === 'directory' ? 'folder' : 'file';
      const m = modal({ title: `Create ${label}`, body: `<label class="label">${label[0].toUpperCase() + label.slice(1)} name</label><input id="nm" class="input" placeholder="${t === 'directory' ? 'config' : 'index.js'}"><p id="err" class="text-xs text-red-300 mt-1 hidden"></p>`,
      ...
      const submit = async () => {
        ...
        try { await api.post(`/files/${current}/create`, { path, type: t }); m.close(); toastSuccess(`${label} created`); list(); }
