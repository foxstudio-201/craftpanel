/* Players tab (Minecraft) — real RCON moderation + whitelist/ban lists. */
(function () {
  window.ServiceTabs = window.ServiceTabs || {};

  window.ServiceTabs.players = async function ({ content, server }) {
    const { escapeHtml, confirmDialog, toastSuccess, toastError } = ui;
    const current = server.id;
    let tab = 'all', search = '', players = [];
    ...
    return () => realtime.off('players:update', load);
  };
})();
