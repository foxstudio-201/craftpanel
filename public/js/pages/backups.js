/* Backups — real tar.gz backup manager per server. */
Layout.mount(async (content) => {
  ...
  content.innerHTML = `
    <div class="glass glass-card p-4 mb-4 flex flex-wrap items-center gap-3">
      ...
      <button id="new" class="btn btn-primary ml-auto"><i data-lucide="archive" class="w-4 h-4"></i> Create Backup</button>
    </div>
    ...
    <p class="text-xs text-slate-500 mt-3">Automatic & scheduled backups: create a <a href="/schedules" class="text-brand-400 hover:underline">schedule</a> with the “backup” action.</p>`;
