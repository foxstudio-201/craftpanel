/* Schedules — real cron automation per server. */
Layout.mount(async (content) => {
  ...
  content.innerHTML = `
    <div class="glass glass-card p-4 mb-4 flex flex-wrap items-center gap-3">
      ...
      <button id="new" class="btn btn-primary ml-auto"><i data-lucide="plus" class="w-4 h-4"></i> New Schedule</button>
    </div>
  ...
  document.getElementById('new').addEventListener('click', () => {
    const m = modal({ title: 'New schedule', body: `
      <form id="f" class="space-y-3">
        ...
        <div id="cmd-wrap" class="hidden"><label class="label">Command (RCON)</label><input name="payload" class="input font-mono" placeholder="say Restarting soon!"></div>
      </form>`,
